...
for _ in range(10):
    ...
    result = subprocess.getoutput(f'./bin/jacobi_seq 10240 1 1')
...
for nw in x:
    ...
    for _ in range(10):
        print("iter", _)
        # Run always with the same seed to have the same matrix
        result = subprocess.getoutput(f'./bin/jacobi_par 10240 1 {nw}')
        tokenized = result.split()
...
with open("./test/results/10240.txt", "w+") as f:
    f.write("Workers: ")
    f.write(str(x))
    f.write("\nSequential:\n")
    f.write(str(y_seq))
...
with open("./test/results/10240.txt", "r") as f:
    print(f.read())
