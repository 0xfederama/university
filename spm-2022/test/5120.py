...
for _ in range(10):
    ...
    result = subprocess.getoutput(f'./bin/jacobi_seq 5120 1 1')
...
for nw in x:
    ...
    for _ in range(10):
        # Run always with the same seed to have the same matrix
        result = subprocess.getoutput(f'./bin/jacobi_par 5120 1 {nw}')
        tokenized = result.split()
...
with open("./test/results/5120.txt", "w+") as f:
    f.write("Workers: ")
    f.write(str(x))
    f.write("\nSequential:\n")
    f.write(str(y_seq))
...
with open("./test/results/5120.txt", "r") as f:
    print(f.read())
